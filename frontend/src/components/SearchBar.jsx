    ...
    <div className="border-t border-b bg-slate-50 dark:bg-slate-800 text-center transition-colors">
      <div className="inline-flex items-center justify-center border border-slate-300 dark:border-slate-600 px-5 py-2 my-5 mx-3 rounded-full w-3/4 sm:w-1/2 bg-white dark:bg-slate-700">
        <input
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="flex-1 outline-none bg-transparent text-sm text-slate-900 dark:text-white placeholder:text-slate-500 dark:placeholder:text-slate-400"
          type="text"
          placeholder="Search"
        />
        <img
          className="w-4 opacity-60 dark:opacity-40"
      ...
      <img
        ...
        className="inline w-3 cursor-pointer opacity-60 dark:opacity-40 hover:opacity-100 transition-opacity"
